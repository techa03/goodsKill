...
本项目为模拟秒杀项目，提供统一秒杀模拟请求接口，技术上整体采用SpringMVC + Mybatis持久层框架，采用Dubbo3.x+原生RestClient完成服务间接口调用，服务注册发现以及配置中心使用Nacos，支持数据库分库分表、分布式事务，使用状态机完成数据状态间的转换（基于Spring Statemachine实现）。
...
* SpringBoot接入OpenTelemetry：`https://spring.io/blog/2025/11/18/opentelemetry-with-spring-boot`
