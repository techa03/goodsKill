...
[WARNING]
docker-compose启动MySQL镜像时会自动执行初始化脚本，如已执行过上一步本步骤可跳过；另外Nacos镜像使用了MySQL数据库保存配置信息，MySQL启动时会自动导入微服务配置
...
为什么有些微服务的application.yml是空的也能正常启动？::

部分微服务的配置已转移至nacos配置中心，可访问 `http://nacos:8848/nacos` 自行修改
