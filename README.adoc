...
本项目为模拟秒杀项目，提供统一秒杀模拟请求接口，方便自行测试各个秒杀方案的执行速度。另外支持OAuth2.0第三方授权登录（目前可通过Gitee，Github进行授权）。 技术上整体采用SpringMVC + Mybatis持久层框架，采用Dubbo3.x{empty}footnote:[由于SpringCloudAlibaba官方暂未支持Dubbo 3.x，本项目采用dubbo-spring-boot-starter集成]，服务注册发现以及配置中心使用Nacos，支持数据库分库分表、分布式事务。由于个人精力有限，最新代码已移除登录以及后台管理页面，只保留了核心秒杀模拟测试API接口。

== 💎分支介绍

``dev_gradle``分支为使用gradle构建工具管理项目依赖（已停更），``dev_maven``分支对应maven构建工具（Spring Framework版本4.x，已停更），``master``分支基于最新Spring Cloud + Spring Boot 3.x体系构建。本项目功能目前比较简陋且有很多不完善的地方，真实的秒杀场景远比本项目中的实现方式复杂，本项目省略了真实场景中的部分技术实现细节，目前仅作学习参考之用，如果觉得本项目对你有帮助的请多多star支持一下👍~~~~。
____
